...
RSpec.describe Validator do
  ...
  context 'params are invalid' do
    it 'returns array of error messages' do
      ...
      expect(described_class.call(data, **current_params)).to include(
    ...
    context 'when many errors in one field' do
      it 'returns only first error' do
        ...
        expect(described_class.call(data, **current_params)).to include(domain: 'Please enter domain')
        expect(described_class.call(data, **current_params)).not_to include(domain: 'Domain not found')
      end
    end
  end
end
